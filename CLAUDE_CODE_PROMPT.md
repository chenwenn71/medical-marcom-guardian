# Claude Code prompt — build the CadenSee standalone demo

Paste everything below into Claude Code inside an empty project folder.

---

Create a Vite + React app that deploys to Vercel's free tier. It is a live medtech marketing-compliance demo. A React frontend calls one Vercel serverless function that holds the Anthropic API key (the key must never reach the browser). Model: Claude Sonnet 4.6.

Create exactly these files with exactly this content. Do not add, rename, or restructure anything. Do not install or run anything. After writing the files, stop.
...
import CadenSeeDemo from "./App.jsx";
...
After the files exist, the human will:
1. Push the repo to GitHub.
2. Import it to Vercel (auto-detects Vite, no config needed).
3. Add an Environment Variable `ANTHROPIC_API_KEY` in Vercel project settings.
4. Deploy.

Do not commit any API key. The key only ever lives in the Vercel env var.
